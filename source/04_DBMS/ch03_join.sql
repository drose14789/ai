...
SELECT EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, D.DEPTNO, DNAME, LOC
    ...
    SELECT ENAME 이름, SAL 급여, E.DEPTNO, LOC
        FROM EMP E, DEPT D
...
SELECT ENAME 이름, SAL 급여, GRADE 등급, E.DEPTNO 부서번호, DNAME 부서이름, LOC 근무지
    FROM EMP E, DEPT D, SALGRADE S
    WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
        AND COMM IS NOT NULL;
-- 탄탄2. 이름, 급여, 입사일, 급여등급
SELECT ENAME 이름, SAL 급여, HIREDATE 입사일, GRADE 등급
    FROM EMP E, DEPT D, SALGRADE S
    WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL;
-- 탄탄3. 이름, 급여, 급여등급, 연봉, 부서명을 부서명순으로 정렬하여 출력
    -- 부서가 같으면 연봉순. 연봉=(sal+comm*12 com이 null이면 0
SELECT ENAME 이름, SAL 급여, GRADE 등급, (SAL + NVL(COMM,0))*12 연봉, DNAME 부서이름
    FROM EMP E, DEPT D, SALGRADE S
    WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
        ORDER BY DNAME, (SAL + NVL(COMM,0))*12 DESC;
-- 탄탄4. 이름, 업무, 급여 등급, 부서코드, 부서명 출력. 급여가 1000~3000사이.
    -- 정렬조건 : 부서별, 부서 같으면 업무별, 업무같으면 급여 큰순
SELECT ENAME 이름, JOB 업무, GRADE 등급, E.DEPTNO 부서코드, DNAME 부서명, SAL
    FROM EMP E, DEPT D, SALGRADE S
    WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
