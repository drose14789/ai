...
SELECT * FROM EMP; -- EMP 테이블의 모든 열, 모든행

-- 2. 특정 열만 출력
DESC EMP;
    -- EMP테이블의 구조
SELECT EMPNO, ENAME, SAL, JOB, HIREDATE FROM EMP; -- EMP테이블의 SELECT절에 지정된 열만 출력
SELECT EMPNO AS "사 번", ENAME AS "이름", SAL AS "급여", JOB AS "직책" 
    FROM EMP; -- 열 이름에 별칭을 두는 경우 EX. 열이름 AS "별칭", 열이름
...
SELECT EMPNO, ENAME, SAL "예전월급", SAL*1.1 "현재월급" FROM EMP;
    ...
    SELECT ENAME, EMPNO, NVL(TO_CHAR(MGR),'CEO') FROM EMP;
    DESC EMP;
